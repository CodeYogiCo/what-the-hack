...
public class NotificationController {
    ...
    public @ResponseBody ApiResponseEntity<NotificationResponseDTO> push(@RequestBody NotificationDataDTO pushData, HttpServletRequest request) throws Exception {
        ...
        @RequestMapping(value = "/user/undelivered/{subscriptionId}", method = RequestMethod.GET, produces = { "application/json" })
        @ResponseStatus(HttpStatus.OK)
        public @ResponseBody ApiResponseEntity<List<NotificationDataDTO>> getNotificationDetails(@PathVariable ("subscriptionId") String registrationId, HttpServletRequest request) throws Exception {
            long startTime = System.currentTimeMillis();
            ApiResponseEntity<List<NotificationDataDTO>> response = new ApiResponseEntity<>();
            List<NotificationDataDTO> notficationData =notificationService.getNotificationDetails(registrationId);
